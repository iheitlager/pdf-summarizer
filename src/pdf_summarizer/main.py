...
class Upload(db.Model):
    ...
    file_hash = db.Column(db.String(64), unique=True, index=True)  # SHA256 hash for caching
...
def index():
        ...
        try:
            ...
            for file in files:
                ...
                if cached_upload:
                    ...
                    log_processing(app.logger, original_filename, page_count, len(text), processing_time)
            ...
            app.logger.info(f"Batch processing completed: {len(processed_ids)} files, {cached_count} cached")
    ...
    recent_uploads = Upload.query.filter_by(session_id=session_id).order_by(Upload.upload_date.desc()).limit(10).all()
...
def my_uploads():
    ...
    session_id = get_or_create_session_id()
    uploads = Upload.query.filter_by(session_id=session_id).order_by(Upload.upload_date.desc()).all()
